...
```{r plot horses, fig.width=10, fig.height=3, fig.cap="Horse populations in Canada for all provinces between 1940 and 1972", out.width="100%"}
...
max_horses <- horse_pop %>% 
  ...
  summarize(Maximum = max(Value),
            Minimum = min(Value))
max_horses |> knitr::kable(caption = "Maximum and minimum number of horses for each province between 1940 and 1972")
```

Below we zoom in and look at the province of `r params$province`:

```{r plot province, fig.width=5, fig.height=4, fig.cap="Number of horses for province of focus", out.width="90%"}
